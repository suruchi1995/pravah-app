...
export default function Handshake() {
  ...
  const totalRisk = data.reduce((a, r) => a + r.revenue_at_risk, 0)
  const gaps = data.filter(r => r.gap_qty > 0).length
    ...
    <>
      ...
      <div className="p-8 space-y-5">
        <div className="flex gap-4">
          ...
          <div className="card p-5 flex-1"><div className="text-xs uppercase text-slate2">SKU·DC with a Gap</div><div className="font-display text-3xl mt-1">{gaps}<span className="text-lg text-slate2">/{data.length}</span></div></div>
        </div>
        <div className="card p-5"><Grid rows={data} columns={cols} height={520} /></div>
      </div>
    </>
  )
}
